...
from .metrics import fmt_pct, fmt_curr, fmt_num
...
def generate_svg_chart(equity_curve: list, spy_equity_curve: list, drawdown_series: list) -> tuple:
...
def build_text_report(m, config: TradingConfig) -> str:
    ...
    lines.append(f" Initial Capital: ${config.starting_capital:,.2f}  |  Position Sizing: {config.position_size_pct:.0%} per trade  |  Rf Rate: {config.risk_free_rate_annual:.2%}")
    lines.append("═" * 85)

    lines.append("\n📝 SIMULATION ASSUMPTIONS & SETTINGS")
    lines.append("─" * 85)
    sim_mode = getattr(config, 'simulation_mode', 'N/A')
    max_pos = getattr(config, 'max_open_positions', 'N/A')
    cash_res = getattr(config, 'cash_reserve_pct', 0.0)
    lines.append(f" Simulation Mode:        {sim_mode}")
    lines.append(f" Position Sizing:        {config.position_size_pct:.1%}")
    lines.append(f" Max Open Positions:     {max_pos}")
    lines.append(f" Cash Reserve:           {cash_res:.1%}")
    
    costs_enabled = getattr(config, 'enable_trading_costs', False)
    lines.append(f" Trading Costs Enabled:  {'Yes' if costs_enabled else 'No'}")
    if costs_enabled:
        comm = getattr(config, 'commission_per_share', 0.0)
        slip = getattr(config, 'slippage_bps', 0.0)
        borrow = getattr(config, 'short_borrow_rate_annual', 0.0)
        lines.append(f"   - Commission:         ${comm:.4f} per share")
        lines.append(f"   - Slippage:           {slip} bps")
        lines.append(f"   - Short Borrow Rate:  {borrow:.1%}")

    lines.append("\n📉 TRADING COSTS SUMMARY")
    lines.append("─" * 85)
    skipped = m.get('skipped_trades', 0)
    tot_comm = m.get('total_commissions', 0.0)
    tot_slip = m.get('total_slippage', 0.0)
    tot_borrow = m.get('total_borrow_costs', 0.0)
    lines.append(f" Skipped Trades:         {skipped}")
    lines.append(f" Total Commissions:      ${tot_comm:,.2f}")
    lines.append(f" Total Slippage:         ${tot_slip:,.2f}")
    lines.append(f" Total Borrow Costs:     ${tot_borrow:,.2f}")

    lines.append("\n📊 1. CAPITAL & RETURN PERFORMANCE")
    lines.append("─" * 85)
    lines.append(f"{'Metric':<35} {'YOUR BOT':<22} {'S&P 500 (' + config.benchmark_ticker + ')':<20} {'SPREAD / ACTIVE':<15}")
    lines.append("─" * 85)
    ...
    lines.append("─" * 85)
    lines.append(f"{'Metric':<35} {'YOUR BOT':<22} {'S&P 500 (' + config.benchmark_ticker + ')':<20} {'STATUS / REMARK'}")
    lines.append("─" * 85)
    ...
    lines.append("─" * 85)
    lines.append(f"{'Ratio':<35} {'YOUR BOT':<22} {'S&P 500 (' + config.benchmark_ticker + ')':<20} {'BENCHMARK THRESHOLD'}")
    lines.append("─" * 85)
    ...
    lines.append("\n📈 5. BENCHMARK (" + config.benchmark_ticker + ") FACTOR REGRESSION & ATTRIBUTION")
    lines.append("─" * 85)
    ...
    lines.append(f" Current Bot Setting:                {config.position_size_pct:.1%} of bankroll per trade")
    if not np.isnan(m['kelly_fraction']):
        ...
        elif config.position_size_pct > m['kelly_fraction']:
            lines.append(f" ⚠️  WARNING: You are OVER-BETTING ({config.position_size_pct:.1%} > Full Kelly {fmt_pct(m['kelly_fraction'])}), causing high risk of ruin.")
        elif config.position_size_pct > m['half_kelly']:
            lines.append(f" ℹ️  NOTE: Sizing is between Half-Kelly ({fmt_pct(m['half_kelly'])}) and Full Kelly ({fmt_pct(m['kelly_fraction'])}).")
        else:
            lines.append(f" ✅ SAFE: Current sizing ({config.position_size_pct:.1%}) is conservative and within Half-Kelly limits.")
    ...
    if diff > 0:
        lines.append(f" ✅ OUTPERFORMANCE: Your bot beat S&P 500 ({config.benchmark_ticker}) by ${diff:,.2f} (+{m['naive_alpha']:.2%} active return).")
    else:
        lines.append(f" ❌ UNDERPERFORMANCE: Your bot lagged S&P 500 ({config.benchmark_ticker}) by ${abs(diff):,.2f} ({m['naive_alpha']:.2%} active return).")
...
def build_markdown_report(m, sim_df, config: TradingConfig) -> str:
    ...
    md.append("")
    md.append(f"> **Period**: `{m['start_date'].strftime('%Y-%m-%d')}` to `{m['end_date'].strftime('%Y-%m-%d')}` ({m['total_days']} days / {m['years']:.2f} yrs) &nbsp;|&nbsp; **Initial Capital**: `{fmt_curr(config.starting_capital)}` &nbsp;|&nbsp; **Position Sizing**: `{config.position_size_pct:.0%}` &nbsp;|&nbsp; **Benchmark**: `{config.benchmark_ticker}`")
    md.append("")
    ...
    md.append("## 📝 Simulation Assumptions")
    md.append(f"- **Mode**: `{sim_mode}`")
    md.append(f"- **Position Sizing**: `{config.position_size_pct:.1%}`")
    md.append(f"- **Max Open Positions**: `{max_pos}`")
    md.append(f"- **Cash Reserve**: `{cash_res:.1%}`")
    md.append(f"- **Trading Costs**: `{'Enabled' if costs_enabled else 'Disabled'}`")
    if costs_enabled:
        comm = getattr(config, 'commission_per_share', 0.0)
        slip = getattr(config, 'slippage_bps', 0.0)
        borrow = getattr(config, 'short_borrow_rate_annual', 0.0)
        md.append(f"  - Commission: `${comm:.4f}/share`")
        md.append(f"  - Slippage: `{slip} bps`")
        md.append(f"  - Short Borrow Rate: `{borrow:.1%}`")
    md.append("")
    ...
    md.append("## 💸 Trading Costs & Execution Summary")
    md.append(f"- **Skipped Trades**: `{skipped}`")
    md.append(f"- **Total Commissions**: `${tot_comm:,.2f}`")
    md.append(f"- **Total Slippage**: `${tot_slip:,.2f}`")
    md.append(f"- **Total Borrow Costs**: `${tot_borrow:,.2f}`")
    md.append("")
    ...
    md.append("")
    md.append(f"| Metric | Your Bot | S&P 500 ({config.benchmark_ticker}) | Active Spread / Advantage |")
    md.append("| :--- | :--- | :--- | :--- |")
    ...
    md.append("")
    md.append(f"| Risk Metric | Your Bot | S&P 500 ({config.benchmark_ticker}) | Description / Notes |")
    md.append("| :--- | :--- | :--- | :--- |")
    ...
    md.append("")
    md.append(f"| Ratio | Your Bot | S&P 500 ({config.benchmark_ticker}) | Target / Guideline |")
    md.append("| :--- | :--- | :--- | :--- |")
    ...
    md.append(f"## 📈 5. Benchmark ({config.benchmark_ticker}) Factor Attribution")
    md.append("")
    beta_sub = f"SE={fmt_num(m['beta_stderr'])} (p={fmt_num(m['beta_pvalue'], 3)})" if not np.isnan(m['beta_stderr']) else "-"
    alpha_sub = f"t={fmt_num(m['alpha_tstat'])} (p={fmt_num(m['alpha_pvalue'], 3)})" if not np.isnan(m['alpha_tstat']) else "-"
    ...
    md.append(f"- **Current Configured Bet Size**: `{config.position_size_pct:.1%}`")
    md.append("")
    if not np.isnan(m['kelly_fraction']):
        if m['kelly_fraction'] <= 0:
            md.append("> [!CAUTION]\n> **Negative Expectancy**: Kelly criterion suggests 0% position size until edge improves.")
        elif config.position_size_pct > m['kelly_fraction']:
            md.append(f"> [!WARNING]\n> **Over-Betting Alert**: Current bet size (`{config.position_size_pct:.1%}`) exceeds Full Kelly (`{fmt_pct(m['kelly_fraction'])}`), creating elevated risk of drawdown/ruin.")
        else:
            md.append(f"> [!NOTE]\n> **Conservative Sizing**: Current sizing (`{config.position_size_pct:.1%}`) is within safe risk bounds.")
    md.append("")
...
def build_html_report(m, sim_df, equity_curve, spy_equity_curve, config: TradingConfig) -> str:
    ...
    kelly_badge = "🔴 Over-Betting Danger" if (not np.isnan(m['kelly_fraction']) and config.position_size_pct > m['kelly_fraction']) else ("🟡 Aggressive (Half - Full Kelly)" if (not np.isnan(m['half_kelly']) and config.position_size_pct > m['half_kelly']) else "🟢 Safe / Conservative (≤ Half-Kelly)")
    ...
    if costs_enabled:
        ...
        slip = getattr(config, 'slippage_bps', 0.0)
        borrow = getattr(config, 'short_borrow_rate_annual', 0.0)
        costs_details = f"<br><strong>Commission:</strong> ${comm:.4f}/share &bull; <strong>Slippage:</strong> {slip} bps &bull; <strong>Borrow Rate:</strong> {borrow:.1%}"
    ...
    <div class="container">
        ...
        <header id="summary">
            <div>
                ...
                <div class="subtitle">
                    Evaluation Window: <strong>{m['start_date'].strftime('%Y-%m-%d')}</strong> to <strong>{m['end_date'].strftime('%Y-%m-%d')}</strong> ({m['total_days']} calendar days / {m['years']:.2f} yrs) &bull; Benchmark: <strong>{config.benchmark_ticker} (S&P 500)</strong>
                </div>
            </div>
            <div style="text-align: right;">
                <span class="badge-pill badge-blue">Position Sizing: {config.position_size_pct:.0%} per trade</span>
                <div style="font-size: 0.8rem; color: var(--text-muted); margin-top: 0.35rem;">Risk-Free Rate: {config.risk_free_rate_annual:.2%} T-Bill</div>
            </div>
        </header>
        ...
        <div class="callout">
            <div class="callout-title">📝 Simulation Assumptions & Trading Costs</div>
            <div class="callout-body">
                <strong>Mode:</strong> {sim_mode} &bull; 
                <strong>Max Positions:</strong> {max_pos} &bull; 
                <strong>Cash Reserve:</strong> {cash_res:.1%} &bull; 
                <strong>Costs Enabled:</strong> {'Yes' if costs_enabled else 'No'}
                {costs_details}
                <br>
                <strong>Skipped Trades:</strong> {skipped} &bull;
                <strong>Commissions:</strong> ${tot_comm:,.2f} &bull;
                <strong>Slippage:</strong> ${tot_slip:,.2f} &bull;
                <strong>Borrow Costs:</strong> ${tot_borrow:,.2f}
            </div>
        </div>
        ...
        <div class="hero-grid">
            <div class="hero-card">
                ...
                <div class="hero-sub">Initial: {fmt_curr(config.starting_capital)} &bull; Profit: <span class="pos">{fmt_curr(m['bot_net_profit'], sign=True)}</span></div>
            </div>
            <div class="hero-card">
        ...
        <table class="kpi-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 14%;">S&P 500 ({config.benchmark_ticker})</th>
        ...
        <!-- ============================================================= -->
        <!-- SECTION 5: BENCHMARK & CAPM FACTOR ATTRIBUTION -->
        <!-- ============================================================= -->
        <div class="section-header" id="capm">
            <div>
                <div class="section-title">📈 5. Benchmark ({config.benchmark_ticker}) Factor Attribution & CAPM</div>
        ...
        <table class="kpi-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>{config.position_size_pct:.1%}</td>
                    <td><span class="badge-pill {'badge-red' if config.position_size_pct > m['kelly_fraction'] else 'badge-green'}">{kelly_badge}</span></td>
                    <td>
...
def save_reports(metrics, sim_df, text_output: str, equity_curve: list, spy_equity_curve: list, config: TradingConfig):
    """Save report.txt, report.md, and report.html and update GitHub Step Summary."""
    # 1. Save text report
    if getattr(config, 'generate_text', True):
        try:
    ...
    if getattr(config, 'generate_markdown', True):
        try:
            md_content = build_markdown_report(metrics, sim_df, config)
    ...
    # 3. Save HTML report
    if getattr(config, 'generate_html', True):
        try:
            html_content = build_html_report(metrics, sim_df, equity_curve, spy_equity_curve, config)
...
def generate_reports(metrics, sim_df, equity_curve: list, spy_equity_curve: list, config: TradingConfig) -> dict:
    """Orchestrator function that builds all reports, prints text, and returns them."""
    text_report = build_text_report(metrics, config)
    print(text_report)
    
    md_report = build_markdown_report(metrics, sim_df, config)
    html_report = build_html_report(metrics, sim_df, equity_curve, spy_equity_curve, config)
